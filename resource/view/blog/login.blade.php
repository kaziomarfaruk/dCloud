    ...
    <div class="row">
        <div style="width:400px;margin:0px auto">
            <div style=" background: #ffffff;border-top: 2px solid #d9d9d9 " class="account-wall">
                ...
                <p class="lead" style="text-align: center;font-size: 12px;padding: 5px">{{$_SESSION["LoginFailed"]}}</p>
                <?php unset($_SESSION["LoginFailed"]); ?>
                <?php  } ?>

                <?php if(isset($_SESSION["RegisterConfirmationSuccess"])){ ?>
                    <p class="lead" style="text-align: center;font-size: 12px;padding: 5px">{{$_SESSION["RegisterConfirmationSuccess"]}}</p>
                    <?php unset($_SESSION["RegisterConfirmationSuccess"]); ?>
                <?php  } ?>

                <?php if(isset($_SESSION['ConfirmationMailMSG'])){ ?>
                    <p class="lead" style="text-align: center;font-size: 12px;padding: 5px">{{$_SESSION['ConfirmationMailMSG']}}</p>
                    <?php unset($_SESSION['ConfirmationMailMSG']); ?>
                <?php  } ?>

                <?php if(isset($_SESSION["PasswordRecover"])){ ?>
                    <p class="lead" style="text-align: center;font-size: 12px;padding: 5px">{{$_SESSION["PasswordRecover"]}}</p>
